		...
		comboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				case 1: //show items in the lab at the moment
					sql = "select * from (select c.name AS Category, p.p_code AS Product, i.sn AS SN, i.notes AS Notes, l.loc_code AS Location from inventory i, product p, category c, location l where c.id = p.c_id AND p.id = i.p_id AND (i.location = l.id OR i.location is null) AND i.availability=1) temp order by temp.Category";
					excelSql = sql;
					excelFile = "items_at_lab_" + today + ".xls";
					Start.showTable(sql, dtm);
					break;
				case 2: //show items at demo
					sql = "select * from (select p.p_code AS Product, i.sn AS SN, m.sender AS Sender, m.receiver AS Receiver, m.organization AS Organization, m.senddate AS 'Send Date', m.promiseddate AS 'Expire Date' from movement m, inventory i, product p, category c where c.id = p.c_id AND p.id=i.p_id AND m.i_id = i.id AND m.io = 0) temp order by temp.Product";
					excelSql = sql;
					excelFile = "items_at_demo_" + today + ".xls";
					Start.showTable(sql, dtm);
					break;
				case 3: //show expired items at demo
					sql = "select * from (select p.p_code AS Product, i.sn AS SN, m.sender AS Sender, m.receiver AS Receiver, m.organization AS Organization, m.senddate AS 'Send Date', m.promiseddate AS 'Expire Date'" +
				...
				case 4: //show full inventory
					sql = "select * from (select c.name AS Category, p.p_code AS Product, i.sn AS SN, i.availability AS Available, i.notes AS Notes from inventory i, product p, category c where c.id = p.c_id AND p.id = i.p_id) temp order by temp.Category";
					excelSql = sql;
		...
		filterField.addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(KeyEvent e) {
				...
				if(filterField.getText().compareTo("")==0)
					...
					String sql = "select c.name, p.p_code, i.sn, i.availability from inventory i, product p, category c " +
		...
		table.addMouseListener(new MouseAdapter() {
			public void mousePressed(MouseEvent e)
			{
				...
				case 0: //show default display table
			        ...
			        popup.add(menuItem);
			        menuItem = new JMenuItem("Send Demo");
			        popup.add(menuItem);
			        menuItem = new JMenuItem("Add to Demo Cart");
			        popup.add(menuItem);
					break;
				case 1: //show at lab
					menuItem = new JMenuItem("Delete");
					...
					break;
				case 2: //show items at demo
					menuItem = new JMenuItem("Receive Demo");
			        popup.add(menuItem);
			        menuItem = new JMenuItem("Delete");
			        popup.add(menuItem);
			        menuItem = new JMenuItem("Edit");
